...
@Component({
  selector: 'app-scrolltable',
  templateUrl: './scrolltable.component.html',
  styleUrls: ['./scrolltable.component.scss']
})
export class ScrolltableComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit(): void {
    let width =  (this.listTable.nativeElement as any)[0].clientWidth;
    (this.listTable.nativeElement as any).width(width)
    (this.listTableHeader.nativeElement as any).width(width)

     (this.tableContent.nativeElement).on('scroll', (e:any) => {
       (this.fixedRowWrapper.nativeElement as any).prop('scrollTop',  (this.tableContent.nativeElement as any).prop('scrollTop'));
       (this.tableHeader.nativeElement as any).prop('scrollLeft', (this.tableContent.nativeElement as any).prop('scrollLeft'))
      (this.tableHeader.nativeElement as any).css({ marginLeft: `${-(this.tableContent.nativeElement as any).prop('scrollLeft')}px` })
    })

    const tds = (this.listTable.nativeElement.getElementsByTagName('tr')[0] as any).children('td');
    if (tds && tds.length > 0) {
      // tds.
    }
    console.log('tds', tds);
