...
@Component({
  selector: 'app-limitvaluesetting',
  templateUrl: './limitvaluesetting.component.html',
  styleUrls: ['./limitvaluesetting.component.scss']
})
export class LimitvaluesettingComponent extends  PageBase implements OnInit {
  ...
  submit()
  { 
    var testprojects:ContactTestProject[]=[];
    this.data.map(
      (x)=>
      {
        testprojects.push(
          {
            ...
            judgementstandard:x.judgementstandard 
          }
        );
      }
    );
    ...
    this.projectservice.updatelimitvalue(testprojects)
    .subscribe(
      (x)=>
      {
        var param:any={};
